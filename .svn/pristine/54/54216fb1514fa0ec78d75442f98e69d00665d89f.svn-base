...
<b><a href="student.html?id=${sid}">Home</a>>Continue Application</b>
		...
		<center>	<h3>Continue Application  </h3>	 
	...
	<b>Following are the Requirement Submitted by Student:</b>
	<table border="1" class="table table-striped">
	<tr>
		...
		<c:forEach items="${gappApplication.additionalReqDetails }" var="req">
			<tr>
				...
				<td><a href="Edit${req.additionalReq.type}.html?sid=${sid}&deptID=${deptID}&AppID=${AppID}&rid=${req.additionalReq.id}&rdid=${req.id}">Edit</a></td>
				<td><a href="Delete.html?sid=${sid}&deptID=${deptID}&AppID=${AppID}&rid=${req.additionalReq.id}&rdid=${req.id}">Delete</a></td>
		...
		<!-- TOEFL Score:<input type="text" name="toefl"> -->
	...
	<div class="well">
	<b>NOTE:</b><br>
	Student should submit all <b>required</b> details by department.<br>
	IF any mandatory requirement is missing Application will not be considered.
	</div>
	
				<br>
				<center>
				<input type="submit" value="Save Application" class="btn btn-primary" name="save" /> 
